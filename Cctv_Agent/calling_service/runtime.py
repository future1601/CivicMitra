...
class CallingServiceRuntime:
    ...
    def create_collect_session(
        ...
        location_prompt_audio_filename: str | None = None
        ...
        try:
            location_audio_path = self.tts_service.generate_speech(
                location_prompt,
                f"collect_location_{token}",
            )
            location_prompt_audio_filename = location_audio_path.name
        except Exception as exc:
            logger.warning(
                "Location prompt TTS generation failed, falling back to Twilio Say: %s",
                exc,
            )
        ...
        session = {
            ...
            "location_prompt_audio_filename": location_prompt_audio_filename,
            "base_url": base_url,
            "issue_retry_count": 0,
